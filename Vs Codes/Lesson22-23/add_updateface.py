...
# Function to update the owner's face
def update_owner_face():
    owner_name = entry_name.get()
    if not owner_name:
        messagebox.showwarning("Input Error", "Please enter the owner's name!")
        return
    ...
    # Save the captured faces and labels
    recognizer.train(faces, np.array(labels))
    recognizer.save(face_data)  # Save the updated face data
    ...
    # Stop the progress bar and update the status
    progress_bar.stop()
    status_label.config(text="Face updated successfully! You can now log in.")
...
# Create a button to update the owner's face
update_button = tk.Button(window, text="Update Owner Face", font=("Arial", 12), command=update_owner_face, bg="#FF5722", fg="white", relief="raised")
